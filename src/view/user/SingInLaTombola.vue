<template>
  <div class="sing-in-container">
    <v-head title="签到成功" self-adaption="SingInLaTombola"></v-head>
    <div class="one">
      <img src="../../assets/icon/money_integral.png" alt="">
      <span>积分+{{ signInData.sign_point }}</span>
    </div>
    <div class="two-time">
      <div v-for="i in computedDate" :class="classShow(i)">
    ...
    <div class="La-tombola-area">
      <div>
        <div id="Scanvas" ref="Scanvas">
          <img v-if="showImg" class="awards" :src="signInData.gift_url" alt="">
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    mounted () {
      ...
      if (th$is.signInData.is_get_gift) {
        ...
        loading.show();
        import('../../../static/js/wScratchPad.min.js').then(jQuery => {
          loading.hide();
...
<style scoped>
  ...
  .two-time-end-item.now-integral .integration, .two-time-item.now-integral .integration {
    position: relative;
    border-color: #fb853b;
    background-color: rgba(251, 133, 59, 0.5);
    color: #fec355;
  }
